...
def home_page():
    ...
    st.write("This app allows users to explore visual features learnt by a Sparse AutoEncoder (SAE) trained on a multimodal modal. I have created an accompanying blog post that contains the technical details of the SAE and the model it was trained on, along with some further analysis. This app is intended to allow non-technical users to find fun features and play around with the SAE. I recommend spending a few minutes looking through some features on the 'neuron navigator' first to get a feel for SAE features before having a go at the 'guess the input image' game. I find it useful to identify interesting SAE features in the game, and then switch back to the neuron navigator and look them up (search with the feature index). Please reach out to me at hugo.fry@fryfamily.co.uk if you have any specific questions or want me to add something to the website!")
    st.subheader("Neuron Navigator")
    st.write("The 'neuron navigator' page contains a list of all the SAE features, and displays the highest activating images of that feature along with some technical details. You can select a neuron from the drop down list or search for a neuron by typing the neuron index into the dropdown list. In the future I am hoping to include custom filters. Please reach out if that's something you'd like!")
    st.subheader("Guess the Input Image!")
    st.write("The 'guess the input image' page is a game that allows users to guess the input image using only the SAE features that fire on the input. The input images are randomly selected from the ImageNet-1k dataset. I recommend looking up interesting SAE features by finding the index from the activation plot, and then searching them in the neuron navigator.")
...
<div class="footer">
<p>Made by Hugo Fry with the help of the <a href="https://www.matsprogram.org/" target="_blank">ML Alignment and Theory Scholars program</a>.</p>
</div>
"""
